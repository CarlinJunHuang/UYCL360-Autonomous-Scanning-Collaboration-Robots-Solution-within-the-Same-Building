...
int lastAvgAngle = 90;  // 初始平均角度
int lastAvgHeight = 125; // 初始平均高度
const int MAX_ANGLE_CHANGE = 25; // 角度变化阈值
const int MAX_HEIGHT_CHANGE = 40; // 高度变化阈值
...
void patrol() {  //1号定时中断
  ...
  while (Serial1.available()) {
    ...
    if (dataReady) {
      ...
      // 检测异常值并处理
      if (abs(angle_int - lastAvgAngle) > MAX_ANGLE_CHANGE) {
        angle_int = 90 + (lastAvgAngle - 90) / 2;
      }
      if (abs(height_int - lastAvgHeight) > MAX_HEIGHT_CHANGE) {
        height_int = 125 + (lastAvgHeight - 125) / 2;
      }
