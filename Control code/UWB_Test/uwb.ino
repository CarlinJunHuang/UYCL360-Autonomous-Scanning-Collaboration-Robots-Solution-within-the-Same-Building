...
const int N = 8;  // 滑动窗口大小
int angleBuffer[N];
int heightBuffer[N];
int bufferIndex = 0;

int avgAngle = 0, avgHeight = 0;
int angle_int = 0, height_int = 0, angle_middle = 0;
float angle = 0, filteredAngle = 0, filteredHeight = 0;
SimpleKalmanFilter kalmanAngle(10.0, 0.1, 0.01); // 这些值需要您根据实际情况调整
SimpleKalmanFilter kalmanHeight(10.0, 0.1, 0.01); // e_mea, e_est, q
...
float Kp_diff = 0.25, Ki_diff = 0, Kd_diff = 0.05;
float PID_Restrict = 200;        // PID输出限幅 
...
/****位置式PID****/
int L_Position_PID(float target_diff, float real_diff) {
    static float diff_Bias, diff_Last_bias = 0, diff_Last_Last_bias = 0; // 定义偏差及上次偏差
    float diff_MPWM; // PID输出值
    diff_Bias = target_diff - real_diff; // 计算偏差

    // 位置式PID控制计算
    diff_MPWM = Kp_diff * diff_Bias                                  // P项
                + Ki_diff * (diff_Bias + diff_Last_bias)            // I项
                + Kd_diff * (diff_Bias - diff_Last_bias);           // D项

    // 限幅
    if (diff_MPWM > PID_Restrict)
        diff_MPWM = PID_Restrict;
    if (diff_MPWM < -PID_Restrict)
        diff_MPWM = -PID_Restrict;

    // 更新偏差值
    diff_Last_Last_bias = diff_Last_bias;
    diff_Last_bias = diff_Bias;
...
void patrol() {  //1号定时中断
  if (avgHeight < 100) {
    ...
    speedall = int(-((avgHeight - 100) / 3.75) - 35);
    if (speedall < -135) {
      speedall = -135;
    }

    speedchange = (avgAngle - 92) * 0.8;
    if (speedchange > 50) {
      speedchange = 50;
    }

    if (avgAngle >= 97) {
      LF_Target = speedall + speedchange;
      LR_Target = speedall + speedchange;
      RF_Target = speedall - speedchange;
      RR_Target = speedall - speedchange;
    } else if (avgAngle <= 89) {
      LF_Target = speedall + speedchange;
      LR_Target = speedall + speedchange;
      RF_Target = speedall - speedchange;
...
void setup()
{
  ...
  memset(angleBuffer, 0, sizeof(angleBuffer)); // 初始化缓冲区
  memset(heightBuffer, 0, sizeof(heightBuffer));
  ...
  time1.setPeriod(330000);                         // Set up period in microseconds，100000us=100ms
...
void loop()
{
  ...
  while (Serial1.available()) {
    ...
    if (dataReady) {
      angle_int = (receivedData[0] << 8) | receivedData[1];
      height_int = (receivedData[2] << 8) | receivedData[3];

      Serial.print(angle_int); Serial.print(",");
      Serial.print(height_int); Serial.print(",");

      // 更新缓冲区
      angleBuffer[bufferIndex] = angle_int;
      heightBuffer[bufferIndex] = height_int;
      ...
      for (int i = 0; i < N; i++) {
        ...
        avgHeight += heightBuffer[i];
      }
      avgAngle /= N;
      avgHeight /= N;

      // 打印滤波后的结果
      Serial.print(avgAngle); Serial.print(",");
