...
//  Serial.print(RF_Velocity); Serial.print(",");
//  Serial.println(RR_Velocity);
...
void patrol() {  //1号定时中断
  ...
  while (Serial1.available()) {
    ...
    if (dataReady) {
      ...
      // 打印开环实验数据
      Serial.print(speeddiff); Serial.print(",");
      Serial.print(angle_int); Serial.print(",");
...
void loop()
{
  nowtime = millis();
  if (speedchange == 0) {
    while (millis() < nowtime + 10 * onesecond){
  ...
  if (nowtime - starttime < 36000) { 
    float rad = nowtime / 20 * PI / 180.0;  // 将角度转换为弧度
    speeddiff =  30 * sin(rad);  // 生成正弦波信号
    speedall = 0;
    }else if (nowtime - starttime >= 36000) {
    speedall = 0;
    speeddiff = 0;
  }

  LF_Target = speedall + speeddiff;
  LR_Target = speedall + speeddiff;
  RF_Target = speedall - speeddiff;
  RR_Target = speedall - speeddiff;
