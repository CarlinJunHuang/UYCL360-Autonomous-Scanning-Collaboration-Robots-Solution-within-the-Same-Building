...
volatile int i, m, n = 0, position[3];
volatile float f;  //调试用的公共变量
unsigned long nowtime = 0;  //时间
const long onesecond = 1000;
const int speedall = -100;

volatile int irbias = 0;
...
float Kp_diff = 1, Ki_diff = 0, Kd_diff = 0.2;
float PID_Restrict = 280;        // PID输出限幅
volatile float LF_Target = 0, RF_Target = 0, LR_Target = 0, RR_Target = 0;  //电机转速目标值,5ms定时器最大可用范围±280，2ms定时器，最大可用范围±120
volatile float diff_value = 0, L_Target = 0, R_Target = 0;
///*********** 限幅************
...
void patrol() {  //1号定时中断
  ...
  irbias = n - groundtruth;

  //以下为4个电机同时计算PID1参数
  diff_value = L_Position_PID(0, irbias); //通过目标值和当前值在PID函数下算出我们需要调整用的PWM值
  LF_Target = speedall + (diff_value / 2);
  RF_Target = speedall - (diff_value / 2);
  LR_Target = speedall + (diff_value / 2);
  RR_Target = speedall - (diff_value / 2);
  
//  Serial1.println(n);   //将含符号位的偏移量发出
//  Serial1.print("bias:");
//  Serial1.println(irbias);
//  Serial1.println(diff_value);
//  Serial1.print("RR_value:");
//  Serial1.println(RR_value);
//  Serial1.println(RR_value);
//  Serial1.println(RF_value);
  
  Serial1.print("RR_target:");
  Serial1.println(RR_Target);

  Serial1.println(LR_Target);
...
void setup()
{
  ...
  time1.setPeriod(7500);                         // Set up period in microseconds，100000us=100ms
...
//  {
    Target_Value = Serial1.readString();  //读取串口字符串
